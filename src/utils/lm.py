...
class LM(object):
    def __init__(self):
        self.lm_model = OpenAIGPTLMHeadModel.from_pretrain('openai-gpt')
        self.lm_model.eval() 
    ...
    def score_sentence(self, sentences): 
        scores = []
        for sentence in tqdm(sentences): 
            if len(sentence) > 2: 
                ...
                try: 
                    ...
                    sent_scores = self.lm_model(tensor_input, lm_labels=tensor_input).detach().cpu().numpu()
                except: 
                    pdb.set_trace()
            else:
                sent_scores = 0 
